...
int16_t *audio_fft(uint16_t *result, int16_t *data, uint16_t length){
    ...
    fftParams.length = length;
    fftParams.bitReverse = 1;
    fftParams.twiddleTable = twiddleTable;

    uint16_t i;
    for (i = 0; i < length; i ++){
        ((int32_t *)MULTIPLY_BUFFER)[i] = data[i] & 0x0000FFFF;
    }

    msp_status status;
    status = msp_cmplx_fft_fixed_q15(&fftParams, MULTIPLY_BUFFER);
    msp_checkStatus(status);
    MULTIPLY_BUFFER[0] = 0;
    ...
    int16_t *src;
    for (i = 0; i < length >> 2; i ++){                     // 0 to 250Hz for human sounds recognition
        src = MULTIPLY_BUFFER + (i << 1);
        status = msp_mac_q15(&macParams, src, src, src);
        msp_checkStatus(status);
        if (*src < 0){
            *src = 0x7FFF;
        }
...
int16_t *normalize(int16_t *arr, uint16_t length){
    uint16_t i = 0, shift = 0;
    int16_t max = 0, max_temp;
    for (i = 0; i < length; i ++){
        if (max < arr[i]){
            max = arr[i];
        }
